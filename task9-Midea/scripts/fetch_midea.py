#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fetch 美的集团 (000333.SZ) daily K-line data via 腾讯行情 API.

Data source: web.ifzq.gtimg.cn (免token, 前复权 qfq)
Spec: specs/midea.yaml

Usage: python scripts/fetch_midea.py

Output:
  - data/midea_daily.csv  (utf-8-sig, ascending by date)
  - data/midea_daily.json (utf-8, indent=2)
"""
import sys
import io
import os
...
# ── Config (from specs/midea.yaml) ──
TENCENT_CODE = "sz000333"      # 美的集团 深圳主板
TS_CODE = "000333.SZ"
NAME = "美的集团"
LIMIT = 250                    # 约1年交易日
PCT_LIMIT = 10                 # 主板 ±10%
...
def save_csv(records, path):
    """Save records as CSV with utf-8-sig BOM."""
...
def main():
    ...
    csv_path = os.path.join(DATA_DIR, "midea_daily.csv")
    json_path = os.path.join(DATA_DIR, "midea_daily.json")
