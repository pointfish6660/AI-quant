#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate K-line technical analysis chart for 长江电力 600900.SH.

Based on task1 build_kline.py pattern.
Template: scripts/chart_template.html
Data: data/cypc_daily.csv
Output: outputs/cypc_kline.html
"""
import csv
import json
import os
...
CSV_PATH = os.path.join(ROOT, "data", "cypc_daily.csv")
...
if dif_[last] and dea_[last]:
    ...
    macd_st = macd_dir = "N/A"

recent_days_desc = []
for r in rows[-5:]:
    cc = r["pct_chg"]
    arrow = "+" if cc >= 0 else ""
    recent_days_desc.append(f'{r["date"]} {arrow}{cc:.2f}%')
...
template = template.replace("__RSI_COLOR__", rsi_color)
...
template = template.replace("__DIF__", f"{dif_[last]:.2f}" if dif_[last] else "N/A")
template = template.replace("__DEA__", f"{dea_[last]:.2f}" if dea_[last] else "N/A")
template = template.replace("__MACD_HIST__", f"{macd_hist[last]:.2f}" if macd_hist[last] else "N/A")
template = template.replace("__BB_UPPER__", f"{bb_upper[last]:.2f}" if bb_upper[last] else "N/A")
template = template.replace("__BB_MID__", f"{bb_mid[last]:.2f}" if bb_mid[last] else "N/A")
